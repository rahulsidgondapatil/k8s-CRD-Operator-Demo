...
type DepSvcResourceLister interface {
	...
	List(selector labels.Selector) (ret []*v1.DepSvcResource, err error)
...
// List lists all DepSvcResources in the indexer.
func (s *depSvcResourceLister) List(selector labels.Selector) (ret []*v1.DepSvcResource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.DepSvcResource))
	})
	return ret, err
}
...
type DepSvcResourceNamespaceLister interface {
	...
	Get(name string) (*v1.DepSvcResource, error)
...
// List lists all DepSvcResources in the indexer for a given namespace.
func (s depSvcResourceNamespaceLister) List(selector labels.Selector) (ret []*v1.DepSvcResource, err error) {
...
// Get retrieves the DepSvcResource from the indexer for a given namespace and name.
func (s depSvcResourceNamespaceLister) Get(name string) (*v1.DepSvcResource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("depsvcresource"), name)
	}
	return obj.(*v1.DepSvcResource), nil
}
